...
server.get('/api/arts/:id', async (req, res) => {
    ...
    if (response === -2) { return res.status(500).send("ID is not on ObjectID format length ex length: 5f682e6a106c1d1d10493a1d "); }
    return res.json(response);
});
...
server.post('/api/arts', (req, res) => {
    const response = artService.createArt(req.body, (art) => {
        return res.status(201).json(art.ops[0]);
    }, (error, item) => {
        if (error === -1) { return res.status(500).json("Id already exists") }
        if (error === -2) { return res.status(500).send("ID is not on ObjectID format length ex length: 5f682e6a106c1d1d10493a1d "); }
...
server.get('/api/artists/:id', async (req, res) => {
    ...
    if (response === -1) { return res.status(404).send("No artist with that ID found"); }
    if (response === -2) { return res.status(500).send("ID is not on ObjectID format length ex length: 5f682e6a106c1d1d10493a1d "); }
    return res.json(response);
});
...
server.get('/api/customers/:id', async (req, res) => {
    ...
    if (response === -1) { return res.status(404).send("No customer with that ID found"); }
    if (response === -2) { return res.status(500).send("ID is not on ObjectID format length ex length: 5f682e6a106c1d1d10493a1d "); }
    return res.json(response);
});
...
server.get('/api/auctions/:id/auction-bids', async (req, res) => {
    const id = req.params.id;
    const response = await customerService.getCustomerAuctionBids(id);
    if (response === -1) { return res.status(404).send("No auction bids found for that ID"); }
    if (response === -2) { return res.status(500).send("ID is not on ObjectID format length ex length: 5f682e6a106c1d1d10493a1d "); }
    ...
    const response = await auctionService.createAuction(req.body, (auction) => {
        ...
        switch (error) {
            ...
            case -3:
                ...
                break;
            case -4:
                return res.status(500).send("artID is not on ObjectID format length ex length: 5f682e6a106c1d1d10493a1d ");
                break;
            default:
...
server.post('/api/auctions/:id/bids', async (req, res) => {
    ...
    const response = await auctionService.placeNewBid(reqId, data.customerId, data.price, (bidresp) => {
        return res.status(200).json(`Bid for auctionId: ${reqId} successfully created`);
    }, (err, item) => {
        switch (err) {
            case -1:
                return res.status(404).send(`No auction with ID: { ${id} } Found.`);
            case -2:
                return res.status(403).send(`That Auction has ended.`);
            case -3:
                return res.status(412).send(`Auction is not higher than the minimum price.`);
            case -4:
